...
function captureTrasseHistoryState() {
  return {
    points:window.trassePoints.map(point => ({lat:point.lat,lng:point.lng})),
    segments:window.trasseSegments.map(segment => ({...segment,domains:segment.domains ? [...segment.domains] : undefined})),
    currentSegStart:window.trasseCurrentSegStart,
    detached:!!window.trasseDetached,
  };
}

function updateTrasseHistoryButtons() {
  const undo = document.getElementById('trasse-undo-btn');
  const redo = document.getElementById('trasse-redo-btn');
  if (undo) undo.disabled = trasseUndoHistory.length === 0;
  if (redo) redo.disabled = trasseRedoHistory.length === 0;
}

function recordTrasseHistory() {
  if (!window.isDrawingTrasse) return;
  trasseUndoHistory.push(captureTrasseHistoryState());
  if (trasseUndoHistory.length > 200) trasseUndoHistory.shift();
  trasseRedoHistory = [];
  updateTrasseHistoryButtons();
}

function restoreTrasseHistoryState(state) {
  setTrassePoints(state.points.map(point => L.latLng(point.lat,point.lng)));
  setTrasseSegments(state.segments.map(segment => ({
    ...segment,
    domains:segment.domains ? [...segment.domains] : undefined,
  })));
  setTrasseCurrentSegStart(state.currentSegStart);
  setTrasseDetached(state.detached);
  redrawTrasse();
...
function snapManualWaermePoint(latlng) {
  ...
  gebaeude.forEach(g => {
    ...
    const distanceM = latlng.distanceTo(center);
    if (distancePx <= 24 && distanceM <= 35 && (!best || distancePx < best.distancePx)) {
...
function findTrasseLineSnap(latlng,excludePointIndex=-1,maxDistancePx=20) {
  ...
  window.trasseSegments.forEach((seg,segIdx) => {
    ...
    for (let pointIndex=seg.start; pointIndex<seg.end; pointIndex++) {
      ...
      const t=denominator
        ? Math.max(0,Math.min(1,((clickPx.x-a.x)*dx+(clickPx.y-a.y)*dy)/denominator))
        : 0;
      const projected=L.point(a.x+t*dx,a.y+t*dy);
      const distancePx=clickPx.distanceTo(projected);
      if (distancePx>maxDistancePx || (best && distancePx>=best.distancePx)) continue;
      best={
        segIdx,pointIndex,t,distancePx,
        latlng:map.containerPointToLatLng(projected),
      };
    }
  });
  return best;
...
export function undoTrassePoint() {
  if (!window.isDrawingTrasse || trasseUndoHistory.length === 0) return false;
  trasseRedoHistory.push(captureTrasseHistoryState());
  restoreTrasseHistoryState(trasseUndoHistory.pop());
  showHint('Letzte Zeichenaktion zurückgenommen. Strg/Cmd+Umschalt+Z stellt sie wieder her.', 3500);
  return true;
}

export function redoTrassePoint() {
  if (!window.isDrawingTrasse || trasseRedoHistory.length === 0) return false;
  trasseUndoHistory.push(captureTrasseHistoryState());
  restoreTrasseHistoryState(trasseRedoHistory.pop());
  showHint('Zeichenaktion wiederhergestellt.', 2500);
  return true;
}
...
export function toggleDrawTrasse(domain) {
  ...
  if (window.isDrawingTrasse) {
    ...
    setTrasseDetached(false);
    showHint(`${trasseDrawDomain === 'strom' ? 'Elektro-Korridor' : manualHeat ? 'Manuelles Wärmenetz' : 'Wärme-Haupttrasse'}: Punkte setzen.${manualHeat ? ' Rechtsklick = Strang beenden · Doppel-Rechtsklick = zurück. Gebäude können direkt angeklickt werden.' : ' Rechtsklick = zurück. „Neuer Strang“ startet einen Abzweig.'}`);
    showTrasseFinishBtn();
    _hideForDraw();
    map.getContainer().style.cursor = 'crosshair';
    map.doubleClickZoom.disable();
...
export function insertTrassePoint(segIdx, afterIndex, latlng,recordHistory=true) {
  const seg = window.trasseSegments[segIdx];
  if (!seg || afterIndex < seg.start || afterIndex >= seg.end) return false;
  if (recordHistory) recordTrasseHistory();
...
export function redrawTrasse() {
  ...
  window.trassePoints.forEach((pt, idx) => {
    ...
    if (draggable) {
      ...
      m.on('dragend', () => {
        if (window._manualWaermeNetzDrawing) {
          const pointPx=map.latLngToContainerPoint(window.trassePoints[idx]);
          const nearBuilding=gebaeude.some(g => g.polygon?.length &&
