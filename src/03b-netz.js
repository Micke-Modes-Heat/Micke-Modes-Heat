...
var WFS_CONFIG = {
  ...
  // SH: kein Eintrag hier — der Dienst erlaubt keine freie BBOX-Abfrage, siehe _wfsFetchBuildingsSH()
...
// Bounding-Boxes und Zuordnung liegen in lib/bundeslaender.js — die PV-Analyse
// braucht dieselbe Zuordnung fürs Landesrecht, ohne das Netz-Modul zu importieren.
export function _detectBundesland(lat, lon) {
  return detectBundesland(lat, lon);
}
...
// ── SH-Sonderfall ─────────────────────────────────────────────────────────
// WFS_SH_ALKIS_vereinf_OpenGBD erlaubt laut Dienstbeschreibung keine freie BBOX-Abfrage,
// sondern nur StoredQueries auf Gemeindeebene (bestätigt: GetFeature mit typeNames+bbox
// liefert 400 "FeatureType ... not available for the WFS-Query-Operation"). Der
// Gemeindeschlüssel wird daher per Reverse-Geocoding (Nominatim, liefert
// de:amtlicher_gemeindeschluessel in extratags) ermittelt und die StoredQuery
// GetGebByGemeinde verwendet — die dann aber die ganze Gemeinde liefert, nicht nur
// den Kartenausschnitt.
var _shAgsCache = new Map();

function _shGemeindeschluessel(lat, lon) {
  var key = lat.toFixed(2) + ',' + lon.toFixed(2);
  if (_shAgsCache.has(key)) return Promise.resolve(_shAgsCache.get(key));
  var url = 'https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=' + lat + '&lon=' + lon + '&zoom=10&extratags=1';
  return fetch(url, { headers: { 'Accept-Language': 'de' }, signal: AbortSignal.timeout(10000) })
    .then(function(r) { if (!r.ok) throw new Error('Nominatim HTTP ' + r.status); return r.json(); })
    .then(function(d) {
      var ags = (d && d.extratags && d.extratags['de:amtlicher_gemeindeschluessel']) || null;
      _shAgsCache.set(key, ags);
      return ags;
    });
}

function _wfsFetchBuildingsSH(bbox4326) {
  var center = [(bbox4326[0] + bbox4326[2]) / 2, (bbox4326[1] + bbox4326[3]) / 2];
  showHint('⏳ Ermittle Gemeinde (SH)…');
  return _shGemeindeschluessel(center[0], center[1])
    .catch(function(err) { console.warn('SH-Gemeindeermittlung fehlgeschlagen:', err.message); return null; })
    .then(function(ags) {
      if (!ags) return null;
      showHint('⏳ Lade Gebäude vom Katasteramt (SH)…');
      var ctrl = new AbortController();
      var timer = setTimeout(function() { ctrl.abort(); }, 30000);
      var url = 'https://service.gdi-sh.de/WFS_SH_ALKIS_vereinf_OpenGBD?service=WFS&version=2.0.0'
        + '&request=GetFeature&storedQuery_id=GetGebByGemeinde&gemeindeschluessel=' + encodeURIComponent(ags);
      return fetch(url, { signal: ctrl.signal })
        ...
        .then(function(txt) {
          var geojson = _parseWfsGml(txt);
          // StoredQuery liefert immer die ganze Gemeinde — auf den angefragten
          // Kartenausschnitt eingrenzen (sonst ggf. tausende Gebäude weit außerhalb).
          geojson.features = geojson.features.filter(function(feat) {
            ...
            var lat = sumLat / ring.length, lng = sumLng / ring.length;
            return lat >= bbox4326[0] && lat <= bbox4326[2] && lng >= bbox4326[1] && lng <= bbox4326[3];
          });
          if (!geojson.features.length) return null;
          showHint('✓ ' + geojson.features.length + ' Gebäude vom Katasteramt (SH)');
          return geojson;
        })
        .catch(function(err) {
          clearTimeout(timer);
          console.warn('WFS-Fehler (SH):', err.message);
          return null;
        });
    });
}
...
export function _wfsFetchBuildings(bbox4326) {
  ...
  if (blId === 'sh') return _wfsFetchBuildingsSH(bbox4326);
...
// UTM → WGS84 (invers, WGS84-Ellipsoid). Grundlage für Landes-WFS, die das angeforderte
// srsName=EPSG:4326 ignorieren und immer im nativen UTM-System antworten (z.B. SH: EPSG:25832).
function _utmToWgs84(easting, northing, centralMeridianDeg) {
  ...
  return { lat: lat * 180 / Math.PI, lng: centralMeridianDeg + lon * 180 / Math.PI };
}
...
function _parseWfsGml(xml) {
  ...
  // srsName aus der Antwort lesen statt blind EPSG:4326 anzunehmen — manche Dienste
  // (z.B. SH) liefern immer UTM-Meter, egal was angefragt wurde.
  var srsEl = doc.documentElement.querySelector('[srsName]');
  var srsName = srsEl ? srsEl.getAttribute('srsName') : '';
  var utmMeridian = /25832/.test(srsName) ? 9 : /25833/.test(srsName) ? 15 : null;
  ...
  members.forEach(function(member) {
    ...
    posLists.forEach(function(pl) {
      ...
      for (var i = 0; i + 1 < nums.length; i += 2) {
        if (utmMeridian !== null) {
          var ll = _utmToWgs84(nums[i], nums[i + 1], utmMeridian);
