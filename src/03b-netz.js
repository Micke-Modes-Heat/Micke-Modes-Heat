...
// Für file://-Kontext: erst direkt versuchen, bei Fehler CORS-Proxies nacheinander.
// Jeder Schritt behandelt sowohl Netzwerk-Fehler als auch HTTP 4xx/5xx als Fehler.
var _CORS_PROXIES = [
  'https://corsproxy.io/?',
  'https://api.allorigins.win/raw?url=',
  'https://thingproxy.freeboard.io/fetch/',
];
function _fetchCors(url, options) {
  function tryProxy(i) {
    if (i >= _CORS_PROXIES.length) return Promise.reject(new Error('Alle Proxies fehlgeschlagen'));
    return fetch(_CORS_PROXIES[i] + encodeURIComponent(url), options)
      .then(function(r) {
        if (!r.ok) return tryProxy(i + 1); // 403/500 usw. → nächster Proxy
        return r;
      })
...
export function _wfsFetchBuildings(bbox4326) {
  ...
  return _fetchCors(url, { signal: ctrl.signal })
    .then(function(resp) {
...
function _enrichBaujahrHamburg(bbox, buildings) {
  ...
  return _fetchCors(url, { signal: AbortSignal.timeout(15000) })
...
function _enrichBaujahrNRW(bbox, buildings) {
  ...
  return _fetchCors(url, { signal: AbortSignal.timeout(10000) })
