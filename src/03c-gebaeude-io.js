...
export function updateTotals(){
  ...
  if (window.elQuartierH) {
    ...
    gebaeude.forEach(g => { if (!isExcluded(g.id)) ts += getGebStromMwh(g); });
...
export function renderGebPvPanel() {
  ...
  const mitFlaeche = gebaeude.filter(g => parseFloat(g.flaeche) > 0);
