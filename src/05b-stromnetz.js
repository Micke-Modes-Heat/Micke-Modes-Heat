...
function _renderCableInspector(panel, edge) {
  ...
  const apply = () => {
    ...
    // Von Hand gesetzt ist nicht mehr geschätzt — damit verschwindet das Kabel
    // aus der Liste „Querschnitt prüfen" und aus dem Bestands-Befund.
...
export function _recalcStromNetzInner() {
    ...
    if (n.type === 'geb') {
      ...
      if (asset) {
        ...
        switch (asset.type) {
          ...
          default:            nm.loadKw = 0; break;
        }
        n.peakLoadKw = Math.abs(nm.loadKw);
        n.isProducer = nm.loadKw < 0;
      }
    }
  });
  ...
  Object.keys(nodeMap).forEach(k => {
    ...
    const isVerb = lkw > 0 && (nodeMap[k].node.type === 'geb' || nodeMap[k].node.isAsset);
    accV[k] = lkw > 0 ? lkw : 0;
    accG[k] = lkw < 0 ? -lkw : 0;
...
export async function loadOsmStrassen() {
  ...
  const bboxString = (south,west,north,east) =>
    [south,west,north,east].map(value => Number(value).toFixed(6)).join(',');
...
export function captureStromNetzState() {
  ...
  return {
    ...
    edges: (window.stromEdges || []).map(e => ({
      ...
      // Geschätzter Querschnitt: ohne diese Felder sähe ein abgeleiteter Wert
      // nach dem Speichern wie ein abgelesener aus.
      qsGeschaetzt: e.qsGeschaetzt || false, qsQuelle: e.qsQuelle || null,
...
export function applyStromNetzState(state) {
  ...
  (state.edges || []).forEach(eData => {
    ...
    if (edge) {
      ...
      edge.qsGeschaetzt = eData.qsGeschaetzt || false;
      edge.qsQuelle = eData.qsQuelle || null;
