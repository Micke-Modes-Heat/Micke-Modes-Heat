 ...
 * MS-Ring schließen.
 *
 * Ein Bestandsplan zeichnet die Ringschlussleitung oft nicht mit — sie läuft
 * im Betrieb ohnehin offen. Topologisch ist sie aber da, und ohne sie sieht das
 * Netz wie ein Stich aus: die (n-1)-Betrachtung in 13g-ms-ring.js findet keinen
 * Ring, und ein Ausfall würde als Totalausfall gerechnet.
 *
 * Ringknoten sind NAP und Schaltanlagen. Trafos bleiben draußen: sie hängen als
 * Abzweig am Ring und wären sonst lauter offene Enden.
 *
 * Geschlossen wird nur, was eindeutig ist: genau zwei offene Enden in einer
 * Komponente. Bei mehr ist das MS-Netz verzweigt — welche Enden zusammen-
 * gehören, kann nur der Plan sagen, nicht der Algorithmus.
 */
function _msRingSchliessen(beteiligteAssets) {
  const ringTypen = new Set(['NAP', 'Schaltanlage']);
  const knoten = ASSETS.items.filter(a => ringTypen.has(a.type));
  const ids = new Set(knoten.map(a => a.id));
  const adj = new Map(knoten.map(a => [a.id, []]));
  for (const e of (window.stromEdges || [])) {
    if (!e.msLevel || !ids.has(e.u) || !ids.has(e.v)) continue;
    adj.get(e.u).push(e.v);
    adj.get(e.v).push(e.u);
  }

  const gesehen = new Set();
  let neu = 0, mehrdeutig = 0;
  for (const start of knoten) {
    ...
    while (stapel.length) {
      const id = stapel.pop();
      komp.push(id);
      for (const nachbar of adj.get(id)) {
        if (!gesehen.has(nachbar)) { gesehen.add(nachbar); stapel.push(nachbar); }
      }
    }
    // Nur Ringe anfassen, an denen dieses Blatt überhaupt beteiligt war — sonst
    // veränderte die Übernahme eines Blattes das MS-Netz einer anderen Ecke.
    if (!komp.some(id => beteiligteAssets.has(id))) continue;
    if (komp.length < 3) continue;                       // Stich, kein Ring möglich
    const enden = komp.filter(id => adj.get(id).length <= 1);
    if (!enden.length) continue;                         // bereits geschlossen
    if (enden.length !== 2) { mehrdeutig++; continue; }
    const u = ASSETS.items.find(x => x.id === enden[0]);
    const v = ASSETS.items.find(x => x.id === enden[1]);
    const e = _verbinde(u, v, { ms: true, cableType: 'NA2XS2Y' });
    if (e) { e.autoGenerated = true; neu++; }
  }
  return { neu, mehrdeutig };
}

/** Hat ein Ring eine Schaltanlage, die als Trennstelle gekennzeichnet ist? */
function _ringOhneTrennstelle() {
  const sas = ASSETS.items.filter(a => a.type === 'Schaltanlage');
  return sas.length > 0 && !sas.some(a => a.props?.trennstelle);
}
...
export function pdApply() {
  ...
  const mutate = () => {
    ...
    const ring = _msRingSchliessen(beteiligteAssets);
    bericht.ringNeu = ring.neu;
    bericht.ringMehrdeutig = ring.mehrdeutig;
  };
  ...
  if (bericht.ringNeu) teile.push(`${bericht.ringNeu} MS-Ringschluss${bericht.ringNeu === 1 ? '' : 'e'}`);
  const offen = [];
